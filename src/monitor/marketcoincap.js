...
const sourceUrl =
  'https://api.coinmarketcap.com/v1/ticker/?limit=10&convert=MXN';

class MarketCoinCap extends CoinBase {
  constructor(fn) {
    super(fn, constants.COIN_MARKET_CAP_KEY, 'coinMarketCap', sourceUrl);
    this.convert = 'MXN';
  ...
  updateData(data) {
    let _data = data.map(record => {
      ...
      const marketCap =
        record[`market_cap_${this.marketcapConvert}`.toLowerCase()];
